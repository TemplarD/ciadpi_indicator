...
### ♾️ Until-found mode (новое в v1.7)

Чекбокс **«Искать до нахождения»** в диалоге поиска отключает лимит
попыток: перебор продолжается, пока какая-нибудь комбинация не пройдёт
проверку URL (или пока вы не нажмёте «Остановить»). Спиннер «Мин.
попыток» задаёт нижний предел — успех раньше него поиск не завершает
(защита от случайного первого прохода), после предела первый же успех
останавливает перебор; итог — самая быстрая из успешных комбинаций.

Комбинации генерируются лениво, волнами (60/120/180…), без повторов —
список не «заканчивается» на фиксированном наборе.

CLI: `--until-found --min-tests 10`

## 🐟 Dual engine: byedpi ↔ nfqws (новое в v1.7)

Меню **«Движок обхода»** переключает два способа обхода DPI:

| | byedpi (ciadpi) | nfqws (zapret) |
|---|---|---|
| Принцип | SOCKS5-прокси | Перехват пакетов (NFQUEUE) |
| Что обходится | Только трафик, направленный в прокси | ВСЕ исходящие TCP 80/443 машины |
| Настройка приложений | Нужна (системный/ручной прокси) | Не нужна |
| Права | Обычный юзер | root-сервис + nftables |

Движки взаимоисключающие: nfqws обрабатывал бы и трафик byedpi (двойной
desync). Переключатель сам останавливает другой движок, снимает его
правила/прокси и запускает выбранный.

Требования для nfqws: пакет `nftables` и исходники zapret в `~/zapret`
(сборка: `git clone https://github.com/bol-van/zapret ~/zapret && make -C ~/zapret/nfq`).
Правила живут в отдельной таблице nft `inet ciadpi` (не трогают таблицу
zapret), очередь `--qnum 200`, перехватываются только первые пакеты
соединения (`ct original packets 1-6`), марка `0x40000000` исключает
собственные пакеты nfqws. Параметры desync — в формате zapret (`--dpi-desync=…`),
свой диалог с примерами и валидацией через `--dry-run`.

Учтите: против провайдеров, детектирующих SNI-подстроку (как YouTube на
некоторых РФ-провайдерах), оба движка бессильны — нужен туннель (xray/hiddify).
